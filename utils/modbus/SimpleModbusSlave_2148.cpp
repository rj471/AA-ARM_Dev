...
#define T1_5_DELAY	(750/SEC_MUL)
#define T3_5_DELAY	(1750/SEC_MUL)
						...
						unsigned int* _regs)
{
	...
	regs = _regs;	
	errorCount = MB_NO_ERROR; // initialize errorCount
	...
	if (baud > 19200){
		...
		T1_5 = ((15000000/baud)/SEC_MUL); // 1T * 1.5 = T1.5
		T3_5 = ((35000000/baud)/SEC_MUL); // 1T * 3.5 = T3.5
	}
}   

unsigned int modbus_update(){
	errorCount=MB_NO_ERROR;
	if (uart1DataAvailable()){
		unsigned char buffer = 0;
		unsigned char overflow = 0;
		// int i;
	
		while (uart1DataAvailable()){
		...
		// serial buffer will be red untill all the data is cleared from the receive buffer.
			if (overflow) 
				...
				if (buffer == BUFFER_SIZE)
					overflow = 1;
				frame[buffer] = uart1Getch();
				buffer++;
			}
			my_delay(T1_5); // inter character time out
		}		
		...
		if (buffer > 7){
			...
			if (id == slaveID || broadcastFlag){ // if the recieved ID matches the slaveID or broadcasting id (0), continue				
				...
				if (calculateCRC(buffer - 2) == crc){ // if the calculated crc matches the recieved crc continue			
					...
					for(index=0;index<holdingRegsSize;index++){
						set_holding_registers_values(index);
					}
					...
					//  crcLo, crcHi
								...
								sendPacket(responseFrameSize+1);
								errorCount = MB_DATA_READ;
							}else	
					...
					errorCount=MB_CHECKSUM_FAILED;
					// errorCount++;
			}else // incorrect id
				errorCount=MB_INCORRECT_ID;
